...
locネットワークとconfネットワークの結果から21のクラスごとに8,732個のデフォルトボックスが得られる. 最終的には確信度の高いものを抽出すればよいが, それだけでは上手く行かない. これは画像中の同じ物体に対して複数のバウンディングボックスが割り当てられることがあるためである. そこで同じ物体を含むバウンディングボックスから, 最も確信度が高いボックスだけを残す処理が必要である. このような処理をNon Maximum Suppression(NMS)と呼ぶ.  
NMSの処理方法について説明する. 1クラスあたり8,732個のデフォルトボックスのうち, 確信度が一定値(default=0.01)を超えたものの座標情報をboxesとする. boxesは(1クラスあたりにつき0.01を超えたDBoxの数, 4)の2階テンソルである. confネットワークの出力から, クラスごとに確信度が一定値(default=0.01)を超えるデフォルトボックスだけを抜き出したものをscoresと呼ぶ. scoresは(1クラスあたりにつき確信度が0.01を超えたDBoxの数, )の1階テンソルである. またIoU値の基準値をoverlap:default=0.5, scoresから確信度の高い順にサンプルを取り出すときの, サンプル数をtop_k:default=200とする. このとき次に示す処理を行う. 4～7の処理によって最も確信度が高いバウンディングボックスと同じ物体を囲んでいるバウンディングボックスを削除することができる. そしてこの処理を繰り返すことで, 別の物体を指しているバウンディングボックスの中で最も確信度が高いバウンディングボックスが残る.

1. クラスごとに次の処理を行う.
2. 全てのバウンディングボックスの面積を求める. 
3. scoresの上位top_k個を抽出してidxに代入する. 確信度が一定値を超えたデフォルトボックスの数がtop_k個より少ない時は, 全てのデフォルトボックスに対して処理を行う.
4. top_k個の確信度の中から, 最も確信度の高いバウンディングボックスをkeepに代入する.
5. idxの要素数が1のとき処理を終了する(終了判定). 
6. 最も確信度の高いバウンディングボックスをidxから削除する.
7. idxからkeepに追加したバウンディングボックスと同じ物体を囲んでいると推察されるものを削除する. 
8. 4に戻る.

同じ物体を囲むバウンディングボックスの判定は, jaccard係数(IoU;Intersection over Union)を用いて行う. IoUは次で計算される. 確信度が最大のバウンディングボックスと, 判定を行いたいバウンディングボックスのIoUを計算して, 値がoverlap以上のときは同じ物体を囲んでいるとしてidxから削除する.

![img](./readme_contents/ssd_IoU.png)

### Detectクラス
locネットワークとconfネットワークの出力を利用して, バウンディングボックスの情報を出力する処理を行うクラスとしてDetectクラスを作成する. 